          ...
          <div className="grid grid-cols-2 gap-3 md:gap-5">
            {cards.map((card, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                variants={fadeUp}
                className="group relative p-5 md:p-8 rounded-xl md:rounded-2xl bg-gradient-to-br from-white/10 to-white/5 border border-white/[0.06] hover:border-white/[0.1] transition-all duration-300 w-full aspect-[4/5] md:aspect-square flex flex-col overflow-hidden"
              >
                {/* Gradient overlay */}
                <div className="absolute inset-0 bg-gradient-to-t from-bg-primary/90 via-bg-primary/40 to-transparent" />

                {/* Icon top left */}
                <span className="relative z-10 text-xl md:text-2xl text-accent">{card.icon}</span>

                {/* Text at bottom */}
                <p className="relative z-10 text-sm md:text-lg text-gray-400 leading-relaxed mt-auto">
